...
- 设置Clover（修改`config.plist`）
  ...
  - 你还可以使用普通的文本文档编辑器或者IDE（如Xcode或者Visual Studio Code）打开`config.plist`对其进行编辑。但是这个方法不够直观，不推荐新手或者代码小白这样操作。
...
- 在[黑果小兵的部落格](https://blog.daliansky.net)按照你的需要下载某个版本的系统镜像文件
- 打开`WinMD5`软件，将下载完成的`iso`镜像文件拖入软件窗口，与网站上提供的`md5`值比对，校验`md5`值是否正确，如不正确，请重新下载（`md5`值相当于一个文件的ID，它的值是唯一的，如果你下载下来的文件的`md5`值与官方提供的不一样，说明你下载的文件可能被修改过或者出错了）
- 找到一个容量为16GB大小或以上的**空U盘**，插入电脑
- 打开`TransMac`软件，鼠标右击你的U盘，点击`Restore With Disk Image`
- 在弹出的窗口中选择你刚才下载好的`iso`文件，点击确认
- 弹出对话框，直接点击`OK`，这时候会**格式化**你的U盘，然后慢慢等待制作完成吧
- 完成以后，系统会提示你要格式化U盘，不必理会，点击取消
...
下面的操作均在Windows下的`DiskGenius`软件中进行。

- 打开`DiskGenius`软件
- 在顶部查看你的硬盘空间分配情况，在最左侧找到你的EFI分区，确认你的EFI分区的空间大于300MB，否则可能会出现macOS无法安装的情况
- 
...
按照上面所说的步骤，如果不出问题，你便在电脑上成功安装了Windows和macOS双系统。如果你只需要macOS的单系统，操作步骤与上面所说有些许不同，但是绝大部分步骤是一样的，唯一的区别在于`给磁盘分区`和`脱离U盘启动`这两部步。如果你在制作安装盘的时候，下载的是daliansky提供的较新系统版本的镜像，或者你在制作完系统启动U盘以后，在`此电脑`中可以看到有诸如`微PE`字样的磁盘，那么下面步骤中的前三步可以省略掉。具体的操作方法如下：
...
- 重启，使用安装盘中微PE的引导启动
- 进入系统后你可以发现界面与Windows10一样，打开你存放在磁盘中的`DiskGenius`，删除你硬盘中Windows使用的分区
- 删除硬盘EFI分区的Windows文件夹
- 将硬盘分区表类型转换为`GUID`格式
- 按照你的需要以及前文所述要求，重新分配你的硬盘分区，并他们格式化
- 接下来就是设置你的BIOS然后安装系统，如果一切顺利，你可以继续下面的步骤
- 重启，使用安装盘中微PE的引导启动
- 安装`UEFIManager`，然后参考上文所说的方法，添加并调整你的引导项
- 大功告成！
